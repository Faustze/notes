...
4. **Доказательство владения приватным ключом (подпись транскрипта).** Сертификат публичен и может быть скопирован — MITM мог бы предъявить чужой валидный сертификат. Поэтому сервер отдельно подписывает **своим собственным** приватным ключом хэш _транскрипта_: полной последовательности сообщений, которыми обменялись в рамках именно этого рукопожатия. Клиент проверяет эту подпись публичным ключом сервера (из сертификата). Это замыкает цепочку доверия: CA подтвердил, что ключ принадлежит домену, а подпись транскрипта доказывает, что сторона на другом конце прямо сейчас реально владеет этим ключом — а не просто переслала копию сертификата.
