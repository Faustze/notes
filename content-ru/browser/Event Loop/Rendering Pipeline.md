...
items.forEach(item => { item.style.width = '100px'; });    // сначала все записи
items.forEach(item => { console.log(item.offsetWidth); }); // затем все чтения — первое чтение пересчитывает, остальные берутся из кэша
```
