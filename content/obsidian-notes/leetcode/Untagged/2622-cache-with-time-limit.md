...
> The class has three public methods: set(key, value, duration): accepts an integer key, an integer value, and a duration in milliseconds.
...
> Constraints: - 0 <= key, value <= 10^9 - 0 <= duration <= 1000 - 1 <= actions.length <= 100 - actions.length === values.length - actions.length === timeDelays.length - 0 <= timeDelays[i] <= 1450 - actions[i] is one of "TimeLimitedCache", "set", "get", and "count" - First action is always "TimeLimitedCache" and must be executed immediately
